...
async function main() {
  ...
  // Verify teacher exists (created by original seed)
  const teacher = await db.prepare('SELECT id FROM users WHERE role = ?').get('teacher');
  if (!teacher) {
    console.error('No teacher user found. Run `npm run seed` first to create base users.');
    process.exit(1);
  }
  ...
  async function buildFullSet(syllabusKey, setNumber) {
    ...
    const seed = setNumber * 100 + syllabusKey.length + 500; // offset by 500 to differ from original seed
    const title = `${syl.label} — Mock Set ${String(setNumber).padStart(2, '0')}`;
    const setInfo = await insertSet.run(title, `Full 200-mark mock exam aligned to ${syl.label}.`, syllabusKey, 'full', 200, 180, 'draft', teacher.id);
  ...
  // ---------- Generate additional sets 13-24 ----------
  // 12 additional sets per syllabus x 4 syllabi = 48 new sets
  let setCount = 0;
  const createdSets = [];
  ...
  console.log('\n=== Generated Additional Question Sets ===');
  console.log(`Total new sets created: ${setCount} (all in DRAFT/unreleased state)`);
  ...
  console.log('\n=== Composition Verification (first additional set of each syllabus) ===');
  for (const sylKey of ['ce_2025', 'ce_2026', 'bph', 'bns']) {
  ...
  console.log('All 48 new question sets are in DRAFT state — teachers can release them selectively.');
  console.log('Run `npm start` to launch the portal and view them.');
}
